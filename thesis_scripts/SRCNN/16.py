...
for epoch_i in range(n_epochs):
    ...
    if (epoch_i % 10) == 0:
        ...
        for example_i in range(n_examples):
            ...
            path1 = "/storage/saurabh/testimg/"
            path2 = "/storage/saurabh/genimg/"


            imsave(path2 + "%d.jpg" %(epoch_i), np.reshape(recon[example_i,:], (height, width, 3)))
            imsave(path + "%d.jpg" %(epoch_i), np.reshape(test_xs[example_i,:], (height, width, 3)))
